...
const HomePage = () => {
  ...
  const { commonTranslate, authTranslate } = useTranslations();
  ...
  return (
    <div className="container mx-auto">
      ...
      {databaseError && (
        ...
        >
          <h3 className="font-bold text-lg">{commonTranslate('databaseConnectionError')}</h3>
          <p>{databaseError}</p>
      ...
      {!isDatabaseReady && dbState !== 'error' && (
        ...
        >
          ...
          <p className="text-gray-600 text-center">
            {commonTranslate('initializingDatabase')}
            <br />
            <span className="text-sm">{commonTranslate('thisMayTakeAMoment')}</span>
          </p>
        </motion.div>
      )}
      ...
      {isAuthLoading && (
        ...
        >
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500 mb-4"></div>
          <p className="text-gray-600 text-center">
            {commonTranslate('checkingAuthStatus')}
          </p>
        </motion.div>
      )}
